...
* Per the WHO, a total of 13,747 cases and 837 deaths have been reported with the current number of daily new cases standing at 3.29 (7-day average). We note that only 1 death was reported since our last update.
* The number of active severe cases requiring hospitalization is estimated at 2 - 4.
* Case Fatality Rate remains high at 6.1%, which may partly due to underreporting. We note another improvement in estimated case reporting rate from 20% to 23%.
* According to the data reported by the MOPH and WHO, the country is on track for containment with fewer than 1 case per 100,000 people in every state. This assessment remains when looking at estimates of true number of cases that adjust for underreporting.
* We note the following measures as currently in place: limits on public gatherings (30% compliance), border closings (50% compliance), partial lockdown (30% compliance), school closures (100% compliance). Please email us (leonardo.milano@un.org) if this information is inaccurate or incomplete as it affects the accuracy of projections.
* The data reported by the authorities suffers from concerning gaps and displays significant variation from day-to-day. This might reflect challenges in data collection and/or batching where multiple days' worth of data are reported at once. For these reasons it is difficult to assess the stability of trends.
* Based on available data, the country saw a peak in cases late May - early June.
* Please keep mind that the current assessment of the situation and all projections depend on complete, accurate data. The model might underestimate the crisis even after accounting for some underreporting. 
"   

key_messages_national_projections <- "
* In 4 weeks, we project that an additional 220 - 438 cases and 14 - 25 deaths will be reported if current NPIs are maintained.
* Projections under the two scenarios (NPIs maintained vs. lifted) are very similar, suggesting that the NPIs currently in place may not suffice to mitigate spread (see sections 2 and 3 for details).
* Daily new cases (the rate at which cases are reported) is projected to increase under both scenarios, resulting in the curve starting to climb back up.
* The number of active severe cases requiring hospitalization is projected to stand at 21 - 39 assuming NPIS are maintained (see section 3).
"

key_messages_subnational_projections <- "
* In two weeks the country is projected to remain at the lowest level of risk (green) although we note increasing trends in daily new cases (see section 4).
* The rate at which daily new cases are added is projected to increase in Khartoum, West Darfur, Sennar and Aj Jazirah.
"
...
increases_nrow <- subnational_projections %>%
                          filter(daily_reported_cases_per_100k_abs_change >= 0) %>%
                          nrow()
...
subnational_projections_ordered <- subnational_projections %>%
          arrange(desc(daily_reported_cases_per_100k_abs_change)) %>% # orders regions from largest to smallest change
          select(ADM1_EN, daily_reported_cases_per_100k_abs_change, daily_reported_cases_per_100k_inTWOweeks)

show_only_increasing_trends <- function(subnational_projections_ordered) {
          subnational_projections_ordered %>%
          mutate(daily_reported_cases_per_100k_abs_change = ifelse(daily_reported_cases_per_100k_abs_change > 0,
                                                                   paste0('+', daily_reported_cases_per_100k_abs_change),
                                                                   daily_reported_cases_per_100k_abs_change)) %>%
          kable(format = 'latex',
          ...
          column_spec(3, width = "2cm", background = ifelse(subnational_projections_ordered$daily_reported_cases_per_100k_inTWOweeks < 1, "#00a67e",
                                             ifelse(subnational_projections_ordered$daily_reported_cases_per_100k_inTWOweeks < 10, "#f8b931",
                                                    ifelse(subnational_projections_ordered$daily_reported_cases_per_100k_inTWOweeks < 25, "#f88c29", "#df431d")))) %>%
