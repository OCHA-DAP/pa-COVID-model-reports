...
* A total of 13,668 cases and 836 deaths have been reported by the WHO. 
* The current number of severe cases requiring healthcare support is estimated at 457 - 708.
* Case Fatality Rate remains high at 6.1%.
* Regions vary in their progress towards containment and therefore in the intensity of control effort needed. Some regions are in the red zone ( = highest level of risk) and could require stay-at-home orders (see section 2). 
* The data reported by the WHO suffers from severe gaps in the last four weeks and displays significant variation
from day-to-day. This might reflect challenges in data collection and/or batching where multiple days’ worth of
data are reported at once. For these reasons it is difficult to assess the stability of trends.
* The case fatality rate remains still significantly higher than expected, suggesting a severe underreporting
of COVID-19 cases. We estimate that only around 3% of the daily COVID-19 cases are reported by the authorities.
* Based on available data, the country saw a peak in cases late May - early June.
"   

key_messages_national_projections <- "
* In the next 4 weeks, the total number of cases is projected to reach 15,224 - 15,939 and the total number of deaths is projected to reach 969 - 1,041 if current NPIs are maintained.
* Lifting of NPIs would lead to a significantly larger increase in cases and deaths (up to 7144 more cases and up to 406 more deaths; see sections 2 and 3 for details).
* The number of severe cases requiring healthcare is projected to increase drastically if NPIs are lifted (see section 3).
"

key_messages_subnational_projections <- "
* Fewer regions are expected to be at a Tipping Point in 2 weeks however many regions will still be battling community spread even if current NPIs are maintained (see section 4).
* The ratio of active cases per 100,000 people is expected to increase in all regions but Red Sea assuming all current NPIs are maintained. We note that Karthoum is projected to reach an alarmingly high incidence rate and may require a more intense control effort (see section 4).
"
...
who_current_cum_cases <- who_data %>%
...
who_current_cum_deaths <- who_data %>%
                      filter(Country == country & 
                      Date_reported == assignment_date) %>%
...
current_reported_incidence_risk_graph_path <- paste0("../Outputs/", country_iso_3_code, "/map_dailycasesreported_per_100k_current.png") 
current_total_incidence_risk_graph_path <- paste0("../Outputs/", country_iso_3_code, "/map_dailycasestotal_per_100k_current.png") # total = reported (as estimated) * case reporting rate
...
projected_reported_incidence_risk_graph_path <- paste0("../Outputs/", country_iso_3_code, "/map_dailycasesreported_per_100k_2w.png")
projected_total_incidence_risk_graph_path <- paste0("../Outputs/", country_iso_3_code, "/map_dailycasestotal_per_100k_2w.png")
...
subnational_projections$daily_cases_reported_per_100k_abs_change <- round(as.numeric(subnational_projections$daily_cases_reported_per_100k_abs_change), 2)
subnational_projections$daily_cases_reported_per_100k_inTWOweeks <- round(subnational_projections$daily_cases_reported_per_100k_inTWOweeks, 3)
```
...
The maps below illustrate regional risk levels as defined by today's incidence rate (daily new cases per 100,000 people as of `r format(assignment_date, "%d %b %Y")`).  On the left are the reported cases while on the right are the total cases as estimated from case reporting rate to adjust for underreporting. The table details the cutoffs for each risk level along with strategies of disease response needed for containment.

&nbsp;

```{r current_incidence_maps, echo = FALSE, fig.show="hold", out.width="50%"}
...
\begin{center}
Case Reporting Rate: `r crr`\%      |      Case Fatality Rate: `r cfr`\%
...
These are common limitations. This report aims to help the reader understand the situation on the ground through comparing and contrasting multiple data sources and estimates. For instance, we present side-by-side the incidence maps of reported cases and total cases, the latter being an estimate of the true number of cases on the ground considering the case reporting rate (ie., how many cases are likely unreported.) The projections are best estimates based on available data.
...
\normalsize The figures below present the historical data on daily new cases and deaths, and their projected trends. Trends are represented by a green line for the "Current NPIs maintained" scenario and a red line for the "No NPIs in place" scenario. Note that deaths typically lag reported cases by 2-8 weeks.
...
The maps below represent the projected risk level of every ADMIN1 region, assuming the current NPIs are maintained. On the left are the cases projected to be reported while on the right are the projected total cases as estimated from case reporting rate to adjust for underreporting. See Section 1 for detail on the relationship between risk level and progress towards virus containment. 

&nbsp;

```{r projected_incidence_maps, echo = FALSE, fig.show="hold", out.width="50%"}
...
Below are the projected incidence rates (= projected daily new cases per 100k people on `r format(projection_date_2w, "%d %b %Y")`) and projected absolute changes in incidence between then and today for every region. Regions are ordered in decreasing order of change magnitude, from largest increase to largest decrease. Note that Incidence Rates are rounded to the third decimal; therefore a 0.000 incidence rate may mean that no new daily cases are projected or that fewer than 1 cases per 100 million people are projected.
...
increases_nrow <- subnational_projections %>%
                          filter(daily_cases_reported_per_100k_abs_change >= 0) %>%
                          nrow()
...
subnational_projections_ordered <- subnational_projections %>%
          arrange(desc(daily_cases_reported_per_100k_abs_change)) %>% # orders regions from largest to smallest change 
          select(ADM1_EN, daily_cases_reported_per_100k_abs_change, daily_cases_reported_per_100k_inTWOweeks) 

show_only_increasing_trends <- function(subnational_projections_ordered) {
          subnational_projections_ordered %>%
          mutate(daily_cases_reported_per_100k_abs_change = ifelse(daily_cases_reported_per_100k_abs_change > 0, 
                                                                   paste0('+', daily_cases_reported_per_100k_abs_change), 
                                                                   daily_cases_reported_per_100k_abs_change)) %>%
          kable(format = 'latex',
          ...
          column_spec(3, width = "2cm", background = ifelse(subnational_projections_ordered$daily_cases_reported_per_100k_inTWOweeks < 1, "#00a67e",
                                             ifelse(subnational_projections_ordered$daily_cases_reported_per_100k_inTWOweeks < 10, "#f8b931",
                                                    ifelse(subnational_projections_ordered$daily_cases_reported_per_100k_inTWOweeks < 25, "#f88c29", "#df431d")))) %>%
