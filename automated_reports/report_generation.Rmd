...
![](visual_assets/double_logo_header_old.png){width=600px}

```{r updated_inputs, echo = FALSE}
country <- 'Afghanistan'
country_iso_3_code <- 'AFG'
assignment_date <- as.Date('2020-07-29')
cfr <- as.numeric('3.5')
npi_projected_cases_4w_LOW <- format(38346, big.mark = ",")
npi_projected_cases_4w_HI <- format(38886, big.mark = ",")
npi_projected_deaths_4w_LOW <-format(1357, big.mark = ",")
npi_projected_deaths_4w_HI <- format(1396, big.mark = ",")

no_npi_projected_cases_4w_LOW <- format(40354, big.mark = ",")
no_npi_projected_cases_4w_HI <- format(41509, big.mark = ",")
no_npi_projected_deaths_4w_LOW <-format(1392, big.mark = ",")
no_npi_projected_deaths_4w_HI <- format(1447, big.mark = ",")

hospitalizations_current_LOW <- format(181, big.mark = ",")
hospitalizations_current_HI <- format(310, big.mark = ",")
npi_hospitalizations_LOW <- format(254, big.mark = ",")
npi_hospitalizations_HI <- format(410, big.mark = ",")
no_npi_hospitalizations_LOW <- format(626, big.mark = ",")
no_npi_hospitalizations_HI <- format(1014, big.mark = ",")
```
...
* A total of 36,471 cases have been reported and 1,271 deaths reported. 
* New reported cases and deaths have declined by 33% and 54% respectively with respect to last week."   

key_messages_national_projections <- "
* A 5-7% projected increase in total cases assuming all current NPIs are maintained. 
* An up to 10% projected increase in total deaths assuming all current NPIs are maintained. 
* Lifting of NPIs would lead to a larger increase in cases and deaths (see sections 2 and 3 for details).
* Cases are expected to grow at a lower rate than projected in the last update. 
* However, this is partially due to delays and gaps in reporting. The number of severe cases is expected to grow despite the predicted lower rate of increase in cases (see section 4)."

key_messages_subnational_projections <- "
* A projected decrease in new cases in all the provinces apart from Parwan Province, assuming all current NPIs are maintained. 
* A projected 4% increase in new cases in Parwan Province, assuming all current NPIs are maintained."
```
...
who_data_filename <- "WHO_data/Data_ WHO Coronavirus Covid-19 Cases and Deaths - WHO-COVID-19-global-data.csv"
...
who_data$date_epicrv <- as.Date(who_data$date_epicrv)
current_cum_cases <- who_data %>%
                      filter(ADM0_NAME == country & 
                      date_epicrv == assignment_date) %>%
                      select(CumCase) %>%
                      as.numeric() %>%
                      format(., big.mark = ",")
...
subnational_projections %>%
          filter(cases_per_100k_change < 0) %>% # keep regions with decreases
          arrange(cases_per_100k_change) %>% # orders regions from largest to smallest absolute change 
          slice_head(n = 5) %>% # keep top 5 areas
          select(ADM1_EN, cases_per_100k_change) %>%
          kable(format = 'latex',
