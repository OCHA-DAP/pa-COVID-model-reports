...
* A total of 3,465 cases and 98 deaths have been reported. 
* New daily reported cases have slowed down since our last update. A single death has been reported.
* The current number of severe cases requiring healthcare support is estimated at 16 - 27.
* The country saw a peak in new cases earlier (May and June). 
* The data reported by the WHO suffers from severe gaps and displays significant variation from day-to-day. For these reasons it is difficult to assess the stability of trends.
"   

key_messages_national_projections <- "
* In the next 4 weeks, the total number of cases is projected to reach 3,666 - 3,707 (a 6-7% increase) and the total number of deaths is projected to reach 108 - 113 (a 9-13% increase) if current NPIs are maintained.
* Lifting of NPIs would lead to a sharp increase in cases and deaths (up to 485 more cases and up to 16 more deaths; see sections 2 and 3 for details).
* The number of severe cases requiring health care support is projected to increase rapidly if NPIs are lifted (see section 3).
"

key_messages_subnational_projections <- "
* Assuming all current NPIs are maintained, subnational projections suggest that Nugaal, Bari, Awdal, Sanaag and Sool will show the largest local decreases (see section 4).
"
```
...
produceIncreasesTable <- function(subnational_projections_dataframe) {
  subnational_projections %>%
          ...
          select(admin1RefN, cases_per_100k_change) %>%
          kable(format = 'latex',
...
# `r increases_result` should be on line 330
```
...
# should be below `r decreases_result`
```
