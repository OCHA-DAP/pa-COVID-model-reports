...
\large \textbf{`r colorize('Projected Changes In Daily New Cases', 'centreBlue')`}
...
increases_nrow <- subnational_projections %>%
                          arrange(desc(cases_per_100k_change)) %>% # orders regions from largest to smallest absolute change 
                          filter(cases_per_100k_change >= 0) %>%
                          nrow()
