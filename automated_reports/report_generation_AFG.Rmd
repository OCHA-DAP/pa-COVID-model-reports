...
header-includes:
    ...
    - \usepackage{float}
#params:
#  assignment_date: !r as.Date('2020-09-01')    
---
...
* A total of 39,096 cases and 1,445 deaths have been reported. 
* The current number of severe cases requiring healthcare support is estimated at 101 - 166 and is stable with respect to our last update.
* Cases had a first peak in early June and deaths had a first peak in early July. 
"   

key_messages_national_projections <- "
* In the next 4 weeks, the total number of cases is projected to reach 39,914 - 40,132 (a 2-3% increase) and the total number of deaths is projected to reach 1,490 - 1,509 (a 3-4% increase) if current NPIs are maintained.
* Lifting of NPIs would lead to a larger increase in cases and deaths (up to 1,130 more cases and up to 41 more deaths; see sections 2 and 3 for details).
* The number of severe cases requiring healthcare is projected to increase, and to increase at least 3 times more if NPIs are lifted (see section 3).
"

key_messages_subnational_projections <- "
* Assuming all current NPIs are maintained, the number of active cases is expected to decrease in all regions (see section 4).
"
```
...
assignment_date <- as.Date("2020-09-23")
...
current_incidence_risk_graph_path <- paste0("../Outputs/", country_iso_3_code, "/current_incidence_risk_graph.png")
...
![](`r current_incidence_risk_graph_path`){width=450px} 

.center[Current Daily New Cases Per 100k People (7-day average)]

```{r containment_progress, echo = FALSE}
kbl(effort_needed_table, 
  ...
  footnote(general_title = "", 
           symbol = c("Daily new cases per 100,000 people"),
           "Source: Harvard Global Health Institute, Edmond J. Safra Center for Ethics and partners",
           footnote_order = c('symbol', 'general')
           )
```


For additional guidance on control effort needed, please see [Key Metrics for COVID Suppression](https://globalepidemics.org/wp-content/uploads/2020/09/key_metrics_and_indicators_v5-1.pdf) by the Harvard Global Health Institute and Edmond J Safra Center for Ethics.

&nbsp;

\large \textbf{`r colorize('Key Figures', 'centreBlue')`}
...
\Large \textbf{`r colorize('3. Subnational Projections', 'centreGreen')`}

{`r colorize('Key Figures', 'centreBlue')`}

\large \textbf{Cases and deaths projections for the next 4 weeks (by `r format(projection_date_4w, "%d %b %Y")`)}

\large \textbf{Projected situation in the next 4 weeks (by `r format(projection_date_4w, "%d %b %Y")`)}
...
Note that deaths typically lag reported cases by 2-8 weeks.

![](`r lifetime_new_cases_graph_path`){width=300px} ![](`r lifetime_new_deaths_graph_path`){width=300px}

![](`r lifetime_cum_cases_graph_path`){width=300px} ![](`r lifetime_cum_deaths_graph_path`){width=300px}
...
\large \textbf{Severe cases projections for the next 4 weeks (by `r format(projection_date_4w, "%d %b %Y")`)}
...
\Large \textbf{`r colorize('4. National Summary', 'centreGreen')`}

\large \textbf{Projected situation in the next 2 weeks (by `r format(projection_date_2w, "%d %b %Y")`)} \footnotesize With current NPIs[^3] maintained

\large \textbf{Active cases per 100,000 people}
...
\large \textbf{Largest Increases and Decreases In Active Cases}
...
produceIncreasesTable <- function(subnational_projections_dataframe) {
  subnational_projections %>%
          ...
          select(ADM1_EN, cases_per_100k_change) %>%
          kable(format = 'latex',
