...
* A total of 39,486 cases and 1,467 deaths have been reported by the WHO. 
* The current number of severe cases requiring healthcare support is estimated at 91 - 147.
* Progress towards containment is insufficient in multiple regions. Regions vary in the intensity of control effort needed to achieve containment (see section 2).
* Cases had a first peak in early June and deaths had a first peak in early July. 
* The data from the WHO and the Ministry of Public health concord well.
"   

key_messages_national_projections <- "
* In the next 4 weeks, the total number of cases is projected to reach 40,194 - 40,432 and the total number of deaths is projected to reach 1,506 - 1,520 if current NPIs are maintained.
* Lifting of NPIs would lead to a larger increase in cases and deaths (up to 942 more cases and up to 29 more deaths; see sections 2 and 3 for details).
* The number of severe cases requiring healthcare is projected to increase, and to increase by least twice as much if NPIs are lifted (see section 3).
"
...
* The ratio of active cases per 100,000 people is expected to increase the most in Ghor, Badghis, Ghazni and to decrease the most in Hirat, Daykundi, Farah assuming all current NPIs are maintained. Note that the 3 regions with the sharpest declines in cases are still projected to be in a red state (= highest risk level) (see section 4).
"
```
...
\large \textbf{`r colorize('Key Figures: Current Cases and Deaths', 'centreBlue')`}[^1][^2]

```{r section2_cumulative_figures, echo = FALSE}

x <- data.frame(Cases = c(mpho_current_cum_cases,current_cum_cases),
                Deaths = c(mpho_current_cum_deaths, current_cum_deaths)) 
...
[^3]: The regional data provided by the Ministry of Public Health are used to generate projections at the subnational level, which are then aggregated to the national level. 

[^4]: \textbf{Non-pharmaceutical interventions - NPIs} are all measures implemented by different actors with the aim of reducing the spread and the impact of COVID-19. The NPIs currently in place are extracted from the [ACAPS database](https://data.humdata.org/dataset/acaps-covid19-government-measures-dataset) and complemented with additional contextual information provided by our partners in the country. 
...
The map below represents the projected risk level of every ADMIN1 region, assuming the current NPIs are maintained. See Section 1 for detail on the relationship between risk level and progress towards virus containment.
...
Below are the projected incidence rates (= projected daily new cases per 100k people on `r format(projection_date_2w, "%d %b %Y")`) and projected absolute changes in incidence between then and today for every region. Regions are ordered in decreasing order of change magnitude, from largest increase to largest decrease. Note that Incidence Rates are rounded to the third decimal; a 0.000 incidence rate does not necessarily mean that no new daily cases are projected.
...
subnational_projections_ordered %>%
          mutate(daily_cases_reported_per_100k_abs_change = ifelse(daily_cases_reported_per_100k_abs_change > 0, paste0('+', daily_cases_reported_per_100k_abs_change), daily_cases_reported_per_100k_abs_change)) %>%
          kable(format = 'latex',
          ...
          pack_rows(index = c('Increasing or Stable' = increases_nrow, 'Decreasing' = decreases_nrow), italic = T) %>%
