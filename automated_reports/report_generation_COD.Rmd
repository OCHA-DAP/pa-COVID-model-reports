...
* No data has been reported since 14 Dec 2020 (MOPH), which means that we have not received any new information since our last report. Please keep mind that the current assessment of the situation and all projections depend on complete, accurate data. The model might underestimate the crisis even after accounting for some underreporting. 

* Per the WHO, a total of {who_current_cum_cases} cases and {who_current_cum_deaths} deaths have been reported with the current number of daily new cases standing at {who_current_newdaily_7daverage_cases} (7-day average). We note a worsening of the situation since our last update.
...
* While most of the country is on track for containment, there is accelerated spread in Kongo-Central (orange risk level), while Kinshasa is at a tipping point (red risk level). More intense control efforts are recommended to counter further spread (see section 2 for detail).
...
* Case numbers are starting to increase in several regions, particularly in Kongo-Central which is already facing accelerated spread (orange risk level)

* In 4 weeks, we project that an additional {npi_projected_deltas_cases_4w_LOW} - {npi_projected_deltas_cases_4w_HI} cases and {npi_projected_deltas_deaths_4w_LOW} - {npi_projected_deltas_deaths_4w_LOW} deaths will be reported if current NPIs are maintained. 

* The trajectory of the outbreak is projected to change based on whether NPIs are maintained: daily new cases are expected to stay roughly constant if NPIs are maintained and to start climbing sharply if NPIs are lifted.

* Lifting the NPIs would lead to a significantly higher number of additional cases and deaths in 4 weeks: up to {no_npi_projected_deltas_cases_4w_MAX} more cases and up to {no_npi_projected_deltas_deaths_4w_MAX} more deaths (see sections 2 and 3 for details). Due to the lag between cases and deaths, a larger number of cases will ultimately result in higher deaths 2 - 8 weeks later.

* The number of active severe cases requiring hospitalization is projected to stay roughly constant assuming NPIs are maintained, and to increase by {no_npi_hospitalizations_delta_LOW} - {no_npi_hospitalizations_delta_HI} if NPIs are lifted (see section 3).
"

key_messages_subnational_projections <- "

* Assuming all current NPIs are maintained, the number of cases is expected to increase in several areas across the country over the next 2 weeks (see section 4).

* Despite a likely decrease in incidence, Kinshasa is projected to remain at a tipping point (red risk level), while Kongo-Central, already facing accelerated spread, is projected to see an increase.
"
...
subnational_projections_ordered <- subnational_projections %>%
          ...
          select(ADM1_FR, daily_cases_total_per_100k_abs_change, daily_cases_total_per_100k_inTWOweeks)
